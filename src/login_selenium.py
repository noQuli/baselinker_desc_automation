...
class CookieManager:
    ...
    def load_cookies(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            if os.path.exists(self.cookies_file_json):
                with open(self.cookies_file_json, 'r') as file:
                    cookies = json.load(file)
                logger.info(f"Cookies loaded from {self.cookies_file_json}")
                return cookies
            else:
...
class LoginElementInteractor:
    ...
    def enter_email(self, email: str):
        """Enter email address"""
        try:
            self.wait.until(EC.presence_of_element_located((By.ID, "loginField")))
            email_field = self.wait.until(EC.element_to_be_clickable((By.ID, "loginField")))
            email_field.clear()
...
class PracujLogin:
    ...
    def __init__(
        ...
        self.account_url = "https://panel-d.baselinker.com/"
        self.login_url = "https://login.baselinker.com/"
        self.email = email
    ...
    def _apply_cookies(self, cookies: List[Dict[str, Any]]) -> None:
        ...
        for cookie in cookies:
            if cookie['domain'] != ".baselinker.com":
                cookie['domain'] = ".baselinker.com"
            try:
    ...
    def _perform_full_login_sequence(self):
        ...
        try:
            ...
            self.wait.until(EC.url_contains(self.account_url))
